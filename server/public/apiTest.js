...
ApiDemo.prototype.createApiClient = function (authUsername, authPassword) {
  this.ravelryApiClient = new RavelryApi(
...
ApiDemo.prototype.addEventListeners = function () {
  ...
  const submitProjectSearch = function () {
    const username = projectListForm.querySelector(
      "input[name='username']"
    ).value;
  ...
  credentialsForm.onsubmit = function () {
    console.log("submit credentialForm");
    const usernameKey = credentialsForm.querySelector(
      "input[name='username_key']"
    ).value;
    const passwordKey = credentialsForm.querySelector(
      "input[name='password_key']"
    ).value;
...
ApiDemo.prototype.renderProjects = function (username, page) {
  ...
  this.ravelryApiClient.projectsList(username, page).then(function (json) {
    ...
    rootElement.innerHTML =
      "<h2>" +
      json.paginator.results +
      " projects found</h2>" +
      "<p> page " +
      json.paginator.page +
      " of " +
      json.paginator.last_page +
      "</p>";
    ...
    nextPageLink.style.display =
      json.paginator.page < json.paginator.last_page ? "block" : "none";
